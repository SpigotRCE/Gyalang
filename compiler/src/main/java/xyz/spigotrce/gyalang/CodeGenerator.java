...
/**
 * Generates JVM bytecode (using OW2 ASM) directly from a class-based Gylang {@link Program} AST.
 * Each Gylang class becomes one JVM class; instance methods, {@code __init__} constructors, and the
 * static {@code Main.main} entry point are mapped onto standard JVM constructs. Stack frames are
 * computed by ASM's {@code COMPUTE_FRAMES} which can be replaced with a class hierarchy based
 * computer.
 */
public class CodeGenerator {
  ...
  private void scanFields(
      Block block, Map<String, String> declared, Map<String, String> literals, Set<String> names) {
  ...
  private void emitFunction(
      ClassWriter cw, ProgramScope scope, ClassDef clazz, FuncDef func, boolean isConstructor) {
    ...
    MethodVisitor mv =
        cw.visitMethod(access, isConstructor ? "<init>" : func.name(), descriptor, null, null);
    mv.visitCode();
  ...
  private void emitAssignment(
    ...
    if (slot == null) {
      slot =
          allocateLocal(
              frame, assignment.name(), typeOf(mv, ctx, frame, scope, fields, assignment.value()));
    }
  ...
  private void emitSetAttr(
    ...
    convert(
        ...
        fields,
        setAttr.value(),
        fields.getOrDefault(setAttr.name(), OBJECT));
    mv.visitFieldInsn(
        Opcodes.PUTFIELD, internal, setAttr.name(), fields.getOrDefault(setAttr.name(), OBJECT));
  }

  private void emitExprStmt(
      MethodVisitor mv,
      MethodCtx ctx,
      MethodFrame frame,
      ProgramScope scope,
  ...
  private void emitForObjectArray(
      MethodVisitor mv,
      MethodCtx ctx,
      MethodFrame frame,
      ProgramScope scope,
  ...
  private void emitRangeCall(
    ...
    mv.visitMethodInsn(
        Opcodes.INVOKESTATIC, RuntimeBuiltins.CLASS_INTERNAL, "range", "(III)[I", false);
  }
  ...
  private int requireArityIn(CallExpr call, String name, int min, int max) {
    ...
    if (n < min || n > max) {
      throw new UnsupportedOperationException(
          name + " expects between " + min + " and " + max + " argument(s)");
    }
    return n;
  }
  ...
  private void emitGetAttr(
    ...
    mv.visitFieldInsn(
        Opcodes.GETFIELD,
        internalName(classOf(receiverType)),
        getAttr.name(),
        fields.getOrDefault(getAttr.name(), OBJECT));
  }
  ...
  private void emitNumericOperand(
      MethodVisitor mv,
      MethodCtx ctx,
      MethodFrame frame,
      ProgramScope scope,
  ...
  private void emitComparison(
    ...
    if (equality && objectEq) {
      ...
      emitExpr(mv, ctx, frame, scope, fields, bin.right());
      mv.visitJumpInsn(
          bin.op() == BinExpr.Op.EQ ? Opcodes.IF_ACMPEQ : Opcodes.IF_ACMPNE, trueLabel);
    } else {
      emitNumericOperand(mv, ctx, frame, scope, fields, bin.left(), floatOp);
      emitNumericOperand(mv, ctx, frame, scope, fields, bin.right(), floatOp);
      if (equality && floatOp) {
        ...
        mv.visitJumpInsn(
            bin.op() == BinExpr.Op.EQ ? Opcodes.IF_ICMPEQ : Opcodes.IF_ICMPNE, trueLabel);
      } else if (floatOp) {
        mv.visitInsn(Opcodes.DCMPL);
        mv.visitJumpInsn(
            switch (bin.op()) {
    ...
    if (call.callee() instanceof GetAttr(Expr receiver, String methodName)) {
      ...
      if (receiverType.startsWith("class:")) {
        emitStaticCall(
            ...
            scope,
            fields,
            receiverType.substring("class:".length()),
            methodName,
            call);
      } else {
  ...
  private void emitConstruction(
    ...
    mv.visitMethodInsn(
        Opcodes.INVOKESPECIAL, internalName(callee), "<init>", "(" + ctorDescriptors + ")V", false);
  }

  private void emitStaticCall(
      MethodVisitor mv,
      MethodCtx ctx,
      MethodFrame frame,
      ProgramScope scope,
    ...
    mv.visitMethodInsn(
        ...
        methodName,
        "(" + paramDescs(method, false) + ")Ljava/lang/Object;",
        false);
  }

  private void emitInstanceCall(
      MethodVisitor mv,
      MethodCtx ctx,
      MethodFrame frame,
      ProgramScope scope,
    ...
    if (isInstanceMethod(method)) {
      ...
      mv.visitMethodInsn(
          ...
          internalName(className),
          methodName,
          "(" + paramDescs(method, true) + ")Ljava/lang/Object;",
          false);
    } else {
  ...
  private void emitBuiltin(
    ...
    switch (callee) {
      ...
      case "input" -> {
        ...
        mv.visitMethodInsn(
            Opcodes.INVOKESTATIC,
            RuntimeBuiltins.CLASS_INTERNAL,
            "input",
      ...
      case "abs" -> {
        ...
        if (f) {
          ...
          mv.visitMethodInsn(
              Opcodes.INVOKESTATIC, RuntimeBuiltins.CLASS_INTERNAL, "abs", "(D)D", false);
        } else {
          convert(mv, ctx, frame, scope, fields, call.arguments().getFirst(), "I");
          mv.visitMethodInsn(
              Opcodes.INVOKESTATIC, RuntimeBuiltins.CLASS_INTERNAL, "abs", "(I)I", false);
        }
      }
      case "min", "max" -> {
        requireArity(call, callee, 2);
        boolean f =
            isFloatArg(mv, ctx, frame, scope, fields, call)
                || isFloatArgAt(mv, ctx, frame, scope, fields, call, 1);
        ...
        mv.visitMethodInsn(
            Opcodes.INVOKESTATIC, RuntimeBuiltins.CLASS_INTERNAL, callee, desc, false);
      }
      case "sqrt", "round" -> {
      ...
      case "sum" -> {
        ...
        emitExpr(mv, ctx, frame, scope, fields, call.arguments().getFirst());
        mv.visitMethodInsn(
            Opcodes.INVOKESTATIC, RuntimeBuiltins.CLASS_INTERNAL, "sum", "([I)I", false);
      }
      case "ord" -> {
  ...
  private void emitVarargsCall(
    ...
    mv.visitMethodInsn(
        Opcodes.INVOKESTATIC, RuntimeBuiltins.CLASS_INTERNAL, methodName, desc, false);
  }
